...
void IdeDrive::displayPartitions() const
{
	struct Partition p;
	for(int i = 0; i < 4; ++i)
	{
		...
		Screen::getScreen().printDebug("Partition %d - start : %u, size %u Go, Bootable : %s", i + 1, p.s_lba, size, (p.bootable == 0x80) ? "True" : "False");
	}
}
...
void Partitions::fillPartition(unsigned int i)
{
	...
	s.printDebug("Start LBA : %u, Size : %u", _partitions[0].s_lba, _partitions[0].size);
}
